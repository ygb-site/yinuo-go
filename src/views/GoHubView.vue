...
<template>
  <div class="min-h-[calc(100vh-5rem)] bg-[#FDFBF7] py-4 sm:py-8 lg:py-10 px-3 sm:px-6 lg:px-8 select-none">
    <div class="max-w-6xl mx-auto space-y-5 sm:space-y-8">
      ...
      <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-600 p-4 sm:p-8 lg:p-9 shadow-xl border-2 sm:border-4 border-white">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div class="space-y-2 sm:space-y-3 text-center md:text-left w-full md:w-auto">
            <div class="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-sm text-white px-2.5 py-0.5 rounded-full text-[11px] font-black shadow-2xs">
            ...
            <div class="bg-white/95 backdrop-blur-md rounded-2xl p-3 sm:p-4 border-2 border-white/90 shadow-md text-left mt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div class="space-y-0.5">
                <div class="flex items-center gap-1.5 text-[11px] font-black text-emerald-700">
      ...
      <div class="bg-white rounded-3xl p-4 sm:p-5 border-2 border-emerald-100 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div class="space-y-3 sm:space-y-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <h2 class="text-lg sm:text-2xl font-cartoon font-bold text-gray-800 tracking-wide">围棋四大核心天地</h2>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div
            ...
            class="group bg-white rounded-3xl p-5 border-2 border-gray-100 shadow-xs hover:shadow-xl hover:border-emerald-300 transition-all duration-200 transform hover:-translate-y-1 active:scale-95 cursor-pointer flex flex-col justify-between"
          >
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <div
              ...
              <div>
                <h3 class="text-base font-cartoon font-bold text-gray-900 group-hover:text-emerald-600 transition-colors flex items-center justify-between tracking-wide">
                  <span>{{ portal.title }}</span>
                  <ArrowRight class="w-4 h-4 text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                </h3>
            ...
            <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-xs font-bold text-gray-500">
              ...
              <span class="text-emerald-600 font-black group-hover:underline flex-shrink-0">进入 →</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-3 sm:space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
          <div class="flex items-center gap-2">
            ...
            <div>
              <h2 class="text-lg sm:text-2xl font-cartoon font-bold text-gray-800 tracking-wide">围棋全功能渐进阶梯</h2>
              <p class="text-[11px] text-gray-400 font-bold hidden sm:block">包含启蒙、死活、吃子对弈、AI对战、打印习题等全部模式</p>
            </div>
          </div>
        ...
        <!-- 3-Column Responsive Grid with Deluxe Card Styling -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-5">
          <div
            ...
            class="relative rounded-3xl p-5 border-2 transition-all duration-200 flex flex-col justify-between cursor-pointer group"
            :class="
              unlockStore.isFeatureUnlocked(feat.id)
                ? 'bg-white border-gray-100 hover:border-emerald-300 shadow-xs hover:shadow-xl transform hover:-translate-y-1 active:scale-98'
          ...
          >
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <div
                  class="w-12 h-12 rounded-2xl bg-gradient-to-tr p-2 text-white shadow-sm flex items-center justify-center text-2xl flex-shrink-0 group-hover:rotate-6 transition-transform"
                  :class="feat.gradient"
                >
                  <span>{{ feat.icon }}</span>
                </div>
                ...
                <div class="flex items-center gap-1.5">
                  <span
                    class="text-[10px] font-black px-2 py-0.5 rounded-full border shadow-2xs"
                  ...
                  <span
                    class="text-[10px] font-black text-white px-2 py-0.5 rounded-full shadow-2xs"
              ...
              <div>
                <h3 class="text-base font-cartoon font-bold text-gray-900 flex items-center justify-between group-hover:text-emerald-600 transition-colors tracking-wide">
                  <span>{{ feat.name }}</span>
                  <ArrowRight
                    v-if="unlockStore.isFeatureUnlocked(feat.id)"
                    class="w-4 h-4 text-emerald-600 opacity-0 group-hover:opacity-100 transition-opacity"
                  />
                </h3>
            ...
            <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-xs font-bold">
              ...
              <div v-else class="text-gray-400 font-bold truncate text-[11px]">
                点击即刻进入体验
              </div>
              ...
              <button
                ...
                "
              >
                <span>{{ unlockStore.isFeatureUnlocked(feat.id) ? '进入体验' : '去解锁' }}</span>
