...
const practiceFeatures = computed(() => {
  return unlockStore.allFeatures.filter(f => f.category === 'practice');
});

const getFeatureStats = (id: string) => {
  if (id === 'arcade') {
    return '最高分: ' + (userStore.arcadeHighScores?.speedCapture || 0) + ' 分';
  }
  if (id === 'tsumego') {
    return '已攻克 ' + tsumegoStore.totalSolvedCount + ' / ' + tsumegoStore.totalPuzzlesCount + ' 题';
  }
  if (id === 'mistakes') {
    const count = userStore.mistakes?.length || 0;
    const resolved = userStore.solvedMistakes?.length || 0;
    ...
    <div class="max-w-6xl mx-auto space-y-5 sm:space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500">
          ...
          <span>/</span>
          <span class="text-slate-800 font-black">练习训练营</span>
        </div>
      </div>

      <!-- Header Hero Card -->
      <div class="bg-gradient-to-r from-purple-500 via-indigo-500 to-rose-500 rounded-3xl p-5 sm:p-8 text-white shadow-lg relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div class="space-y-2 text-center md:text-left">
            <div class="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-black">
              ...
              <span>专项技能修炼场 (Practice Hub)</span>
            </div>
            ...
            <p class="text-xs sm:text-sm text-white/90 font-medium max-w-xl">
              结合闯关进度循序解锁！从60秒极速连击、死活题专项突破到错题弱点消灭，打造围棋基本功！
            </p>
          </div>
          ...
          <div class="flex items-center gap-3 bg-white/15 backdrop-blur-md rounded-2xl p-3 sm:p-4 border border-white/25">
            <div class="text-center">
              <div class="text-[10px] font-black text-white/80">已解锁练习</div>
              <div class="text-xl sm:text-2xl font-black text-white">
                {{ practiceFeatures.filter(f => unlockStore.isFeatureUnlocked(f.id)).length }} / {{ practiceFeatures.length }}
              </div>
            </div>
          </div>
        </div>
