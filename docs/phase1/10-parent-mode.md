...
家长端**不是数据大屏**。它先回答「这周我们一起做了什么」，再回答五个学情问题：
...
第五个问题是当前 `ParentDashboardView` 原先几乎没有回答的，也是家长最需要的。
回衡水（四到六年级）之后，家长最容易变成监工——所以北京这几年，关系必须排在分数前面。
...
```
/parent                  概览（关系在前，结论随后）
├── Together             本周一起做的一件事（对弈 / 共读 / 观察 / 家务）
├── 年级与双轨           学校轴、老家轴、回老家窗口（儿童不可见）
├── 本周一句话结论
├── 本周关注点（1–2 条，带可执行动作）
├── 学习概览（天数 / 时长 / 完成关卡 / 正确率）
├── 能力速览（六维，缺数据显式标注）
...
学习时长目标施压、付费转化模块、儿童端的衡水预习包、一年级奥数入口、
没有观察闭环的「科学学堂」假进度。

### Together 规则

- 完成标准是「一起做了」，打勾即可。
- 不发金币、不进 `AbilityProfile`、不出现在儿童 Today。
- 每周一重置。不补上周的。
- 建议动作优先跳已有闭环：亲子对弈走 `/two-player`。

### 双轨规则

- `schoolTrack` 决定儿童课表跟哪座城市。
- `hometownTrack` + `returnWindow` 只出现在家长端。
- `returnWindow = g4_g6` 时，四年级前家长只看见习惯提醒，不看见第二套作业入口。
- `trackRole` 切到 `hometown` 后，学校轴翻成老家，北京素质课变成保持项。
