...
export class GoAI {
     ...
     */
    static evaluateMove(board, r, c, color) {
     ...
     */
    static selectMove(board, difficulty, color = board.turn) {
     ...
     */
    static getBestMoveHint(board, color = board.turn) {
        const legalMoves = this.getLegalMoves(board, color);
        if (legalMoves.length === 0)
            return null;
        const scoredMoves = legalMoves.map(p => {
